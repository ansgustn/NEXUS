...
def main():
    print("=== Synthesizing all 10 Thinking & Refusal Audio files via python -m edge_tts ===")
    for figure_id in FIGURE_VOICES:
        for category in ["thinking", "refusal"]:
            ...
            print(f"🎙️ [{figure_id}] {category} -> {filename} ({v_conf['voice']} | {v_conf['pitch']} | {v_conf['rate']})...")
            p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if p.returncode == 0 and os.path.exists(out_path) and os.path.getsize(out_path) > 1000:
                print(f"   ✅ Saved {out_path} ({os.path.getsize(out_path):,} bytes)")
            else:
                print(f"   ❌ FAILED: {p.stderr}")
            time.sleep(0.3)

    print("=== All 10 Audio files synthesized! ===")
